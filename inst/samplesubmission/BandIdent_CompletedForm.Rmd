...
<img src='`r system.file('extdata','imb_logo.png', package='cfpscripts')`' style='width:25%; float:left' />
...
**Background species:** `r paste(user_inputs$background_dbs,collapse=",",sep=", ") `
...
if(user_inputs$barcode_acid!=''){
  cat('
<p style="color:red">__Please provide enough material(= two identical band but two 
                  different sticker numbers). One sample will be digested with 
                  trypsin and the other hydrolysed with 3M HCl__</p>')
}

```
```{r  echo=FALSE, comment=NA, results='asis'}
  var_to_readable = list("barcode_acid"="Acid Hydrolysis",
                  "barcode_trypsin"="Trypsin",
                  "barcode_protease"=user_inputs$name_protease)
  var_to_readable <- var_to_readable[grep(".",user_inputs[names(var_to_readable)])]
  ...
  rownames(df)=NULL
  if(length(df) > 0){
    knitr::kable(df, format="markdown", align='c', escape=FALSE)
  }else{
